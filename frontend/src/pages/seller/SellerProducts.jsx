...
import { Plus, Eye, Filter, Edit, Trash2, MoreVertical, CheckCircle, XCircle } from "lucide-react";
import { getSellerOffers, deleteSellerOffer, updateSellerOffer } from "../../api/seller";
...
const SellerProducts = () => {
    ...
    const fetchOffers = useCallback(async () => {
        setLoading(true);
        try {
            const data = await getSellerOffers({ page, limit: 10, search, status: statusFilter });
    ...
    const handleStatusToggle = async (offer, currentStatus) => {
        ...
        try {
            await updateSellerOffer(offer.id, { status: newStatus });
    ...
    const columns = [
            ...
            render: (offer) => (
                <div className="flex items-center gap-3">
                    {offer.product.media?.[0]?.url ? (
                        <img src={offer.product.media[0].url} alt={offer.product.name} className="w-10 h-10 rounded-lg object-cover" />
                    ) : (
                        <div className="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center text-xs text-gray-400">No Img</div>
                    )}
                    <div>
                        <p className="font-medium text-gray-900 line-clamp-1">{offer.product.name}</p>
                        <p className="text-xs text-gray-500">Model: {offer.product.model || 'N/A'}</p>
                    </div>
                </div>
            )
        },
        ...
        {
            header: "Price",
            render: (offer) => <span className="font-medium">{formatCurrency(offer.price)}</span>
        },
        {
            header: "Stock",
            render: (offer) => (
                <span className={`font-medium ${offer.stockQuantity < 5 ? 'text-red-600' : 'text-gray-700'}`}>
                    {offer.stockQuantity}
                </span>
            )
        },
        {
            ...
            render: (offer) => (
                <button 
                    ...
                    {offer.status === 'active' ? <CheckCircle size={12} /> : <XCircle size={12} />}
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-gray-500 text-sm">Manage your product suggestions and inventory.</p>
                </div>
            </div>
